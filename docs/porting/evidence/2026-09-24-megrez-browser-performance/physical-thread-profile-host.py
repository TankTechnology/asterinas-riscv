...
try:
    ...
    payload = base64.b64encode(gzip.compress((root / 'physical-thread-profile-guest.py').read_bytes(),
                                           mtime=0)).decode()
    command('create-transfer', "printf '' > /run/asterinas-thread-profile.b64")
    for index, start in enumerate(range(0, len(payload), 900), 1):
        command(f'transfer-{index}',
                "printf '%s' '" + payload[start:start + 900] +
                "' >> /run/asterinas-thread-profile.b64")
    command('extract-guest',
            'base64 -d /run/asterinas-thread-profile.b64 | gzip -d > /run/asterinas-thread-profile.py')
    output = command(
        'canvas-probe',
        '_pid=$(pgrep -xo firefox); _xpid=$(pgrep -xo Xorg); '
        'nsenter -t "$_pid" -n env BROWSER_PID="$_pid" XORG_PID="$_xpid" '
        'timeout 100 python3 -u /run/asterinas-thread-profile.py', timeout=115,
    )
    (root / 'physical-thread-profile-guest-output.txt').write_text(output)
    chunks = {}
    endings = {}
    for line in output.splitlines():
        match = re.fullmatch(r'THREAD_PROFILE_CHUNK run=(\d+) i=(\d+) data=(\S+)', line)
        if match:
            chunks.setdefault(int(match.group(1)), {})[int(match.group(2))] = match.group(3)
        match = re.fullmatch(r'THREAD_PROFILE_END run=(\d+) parts=(\d+) sha256=([0-9a-f]{64})', line)
        if match:
    ...
    (root / 'physical-thread-profile-result.json').write_text(json.dumps({
    ...
    for sample in samples:
        ...
        print('top_threads', sample['thread_sample']['top_threads'][:3])
finally:
    (root / 'physical-thread-profile-host-commands.json').write_text(json.dumps(records, indent=2))
    if serial is not None:
        (root / 'physical-thread-profile-serial.log.gz').write_bytes(
