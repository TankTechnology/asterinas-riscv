...
try:
    ...
    payload = base64.b64encode(gzip.compress((root / 'physical-video-source-guest.py').read_bytes(),
                                           mtime=0)).decode()
    command('create-transfer', "printf '' > /run/asterinas-video-source.b64")
    for index, start in enumerate(range(0, len(payload), 900), 1):
        command(f'transfer-{index}',
                "printf '%s' '" + payload[start:start + 900] +
                "' >> /run/asterinas-video-source.b64")
    command('extract-guest',
            'base64 -d /run/asterinas-video-source.b64 | gzip -d > /run/asterinas-video-source.py')
    output = command(
        'canvas-probe',
        '_pid=$(pgrep -xo firefox); _xpid=$(pgrep -xo Xorg); '
        'nsenter -t "$_pid" -n env BROWSER_PID="$_pid" XORG_PID="$_xpid" '
        'timeout 100 python3 -u /run/asterinas-video-source.py', timeout=115,
    )
    (root / 'physical-video-source-guest-output.txt').write_text(output)
    chunks = {}
    endings = {}
    for line in output.splitlines():
        match = re.fullmatch(r'VIDEO_SOURCE_CHUNK run=(\d+) i=(\d+) data=(\S+)', line)
        if match:
            chunks.setdefault(int(match.group(1)), {})[int(match.group(2))] = match.group(3)
        match = re.fullmatch(r'VIDEO_SOURCE_END run=(\d+) parts=(\d+) sha256=([0-9a-f]{64})', line)
        if match:
    ...
    (root / 'physical-video-source-result.json').write_text(json.dumps({
    ...
    (root / 'physical-video-source-host-commands.json').write_text(json.dumps(records, indent=2))
    if serial is not None:
        (root / 'physical-video-source-serial.log.gz').write_bytes(
