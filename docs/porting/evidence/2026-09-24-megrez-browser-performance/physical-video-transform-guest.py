...
BASE = 'http://10.100.19.216:17895/video-transform-perf.html'
...
try:
    ...
    created = True
    for run, (mode, width, height) in enumerate((('direct', 1280, 720),
                                                  ('transform', 1280, 720),
                                                  ('transform', 1280, 720),
                                                  ('direct', 1280, 720)), 1):
        ...
        url = f'{BASE}?mode={mode}&w={width}&h={height}&run={run}&revision=transform1'
        ...
        for index, part in enumerate(parts):
            print(f'VIDEO_TRANSFORM_CHUNK run={run} i={index} data={part}', flush=True)
        print(f'VIDEO_TRANSFORM_END run={run} parts={len(parts)} '
        ...
        assert value['result']['totalFrames'] == 300
        assert value['result']['renderedWidth'] == 1280
        assert value['result']['renderedHeight'] == 720
finally:
    if created:
        try:
