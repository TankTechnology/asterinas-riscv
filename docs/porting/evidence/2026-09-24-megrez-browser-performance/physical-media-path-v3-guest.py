...
BASE = 'http://10.100.19.216:17895/media-path-perf.html'
...
try:
    ...
    created = True
    for run, (mode, width, height) in enumerate((('direct', 320, 180),
                                                  ('canvas', 1280, 720),
                                                  ('canvas', 640, 360)), 1):
        ...
        url = f'{BASE}?mode={mode}&w={width}&h={height}&run={run}&revision=3'
        ...
        for index, part in enumerate(parts):
            print(f'MEDIA_PATH_CHUNK run={run} i={index} data={part}', flush=True)
        print(f'MEDIA_PATH_END run={run} parts={len(parts)} '
