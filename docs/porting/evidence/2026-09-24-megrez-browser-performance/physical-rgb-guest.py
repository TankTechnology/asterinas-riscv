...
BASE = 'http://10.100.19.216:17895/rgb-frame-perf.html'
    ...
    for run, (width, height) in enumerate(((640, 360), (1280, 720),
        ...
        while time.monotonic() < deadline:
            ...
            response = client.command('WebDriver:ExecuteScript', {
                ...
                'sandbox': 'asterinas-rgb-perf', 'line': 1,
                'filename': 'asterinas-rgb-perf'})
        ...
        for index, part in enumerate(parts):
            print(f'RGB_CHUNK run={run} i={index} data={part}', flush=True)
        print(f'RGB_END run={run} parts={len(parts)} '
