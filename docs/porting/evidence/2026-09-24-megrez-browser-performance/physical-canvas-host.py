...
try:
    ...
    payload = base64.b64encode(gzip.compress((root / 'physical-canvas-guest.py').read_bytes(),
                                           mtime=0)).decode()
    command('create-transfer', "printf '' > /run/asterinas-canvas.b64")
    for index, start in enumerate(range(0, len(payload), 900), 1):
        command(f'transfer-{index}',
                "printf '%s' '" + payload[start:start + 900] +
                "' >> /run/asterinas-canvas.b64")
    command('extract-guest',
            'base64 -d /run/asterinas-canvas.b64 | gzip -d > /run/asterinas-canvas.py')
    output = command(
        'canvas-probe',
        '_pid=$(pgrep -xo firefox); _xpid=$(pgrep -xo Xorg); '
        'nsenter -t "$_pid" -n env BROWSER_PID="$_pid" XORG_PID="$_xpid" '
        'timeout 100 python3 -u /run/asterinas-canvas.py', timeout=115,
    )
    (root / 'physical-canvas-guest-output.txt').write_text(output)
    chunks = {}
    endings = {}
    for line in output.splitlines():
        match = re.fullmatch(r'CANVAS_CHUNK run=(\d+) i=(\d+) data=(\S+)', line)
        if match:
            chunks.setdefault(int(match.group(1)), {})[int(match.group(2))] = match.group(3)
        match = re.fullmatch(r'CANVAS_END run=(\d+) parts=(\d+) sha256=([0-9a-f]{64})', line)
        if match:
            endings[int(match.group(1))] = (int(match.group(2)), match.group(3))
    samples = []
    for run in range(1, 5):
        if run not in endings or run not in chunks:
            raise RuntimeError(f'canvas run {run} is missing serial chunks')
        count, digest = endings[run]
        if set(chunks[run]) != set(range(count)):
            raise RuntimeError(f'canvas run {run} has missing serial chunks')
        payload = base64.b64decode(''.join(chunks[run][i] for i in range(count)))
        if hashlib.sha256(payload).hexdigest() != digest:
            raise RuntimeError(f'canvas run {run} serial checksum mismatch')
        sample = json.loads(payload)
        if sample['run'] != run:
            raise RuntimeError(f'canvas run {run} label mismatch')
        samples.append(sample)
    (root / 'physical-canvas-result.json').write_text(json.dumps({
    ...
    (root / 'physical-canvas-host-commands.json').write_text(json.dumps(records, indent=2))
    if serial is not None:
        (root / 'physical-canvas-serial.log.gz').write_bytes(
