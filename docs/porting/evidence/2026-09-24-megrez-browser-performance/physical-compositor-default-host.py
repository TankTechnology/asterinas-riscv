...
try:
    ...
    payload = base64.b64encode(gzip.compress((root / 'physical-compositor-guest.py').read_bytes(),
                                           mtime=0)).decode()
    command('create-transfer', "printf '' > /run/asterinas-compositor.b64")
    for index, start in enumerate(range(0, len(payload), 900), 1):
        command(f'transfer-{index}',
                "printf '%s' '" + payload[start:start + 900] +
                "' >> /run/asterinas-compositor.b64")
    command('extract-guest',
            'base64 -d /run/asterinas-compositor.b64 | gzip -d > /run/asterinas-compositor.py')
    output = command(
        'canvas-probe',
        '_pid=$(pgrep -n -x firefox); _xpid=$(pgrep -xo Xorg); '
        'nsenter -t "$_pid" -n env BROWSER_PID="$_pid" XORG_PID="$_xpid" '
        'timeout 100 python3 -u /run/asterinas-compositor.py', timeout=115,
    )
    (root / 'physical-compositor-default-guest-output.txt').write_text(output)
    chunks = {}
    endings = {}
    for line in output.splitlines():
        match = re.fullmatch(r'COMP_CHUNK run=(\d+) i=(\d+) data=(\S+)', line)
        if match:
            chunks.setdefault(int(match.group(1)), {})[int(match.group(2))] = match.group(3)
        match = re.fullmatch(r'COMP_END run=(\d+) parts=(\d+) sha256=([0-9a-f]{64})', line)
        if match:
    ...
    (root / 'physical-compositor-default-result.json').write_text(json.dumps({
    ...
    (root / 'physical-compositor-default-host-commands.json').write_text(json.dumps(records, indent=2))
    if serial is not None:
        (root / 'physical-compositor-default-serial.log.gz').write_bytes(
