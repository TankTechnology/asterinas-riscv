...
try:
    ...
    created = True
    for run, source in enumerate(('clip720.webm', 'clip360.webm',
                                  'clip360.webm', 'clip720.webm'), 1):
        mode, width, height = 'direct', 1280, 720
        ...
        for index, part in enumerate(parts):
            print(f'VIDEO_SOURCE_CHUNK run={run} i={index} data={part}', flush=True)
        print(f'VIDEO_SOURCE_END run={run} parts={len(parts)} '
