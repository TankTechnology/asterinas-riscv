...
static void run_case(const char *name, int op, size_t valid, size_t payload, size_t request)
{
    int listener = socket(AF_INET, SOCK_STREAM, 0);
    if (listener < 0) die("socket");
    struct sockaddr_in address = { .sin_family = AF_INET, .sin_addr.s_addr = htonl(INADDR_LOOPBACK) };
    if (bind(listener, (void *)&address, sizeof(address)) < 0) die("bind");
    socklen_t length = sizeof(address);
    if (getsockname(listener, (void *)&address, &length) < 0) die("getsockname");
    if (listen(listener, 1) < 0) die("listen");
    int sender = socket(AF_INET, SOCK_STREAM, 0);
    if (sender < 0) die("sender socket");
    if (connect(sender, (void *)&address, sizeof(address)) < 0) die("connect");
    int receiver = accept(listener, NULL, NULL);
    if (receiver < 0) die("accept");
    ...
    if (mprotect(buffer + 4096, 4096, PROT_NONE) < 0) die("mprotect");
    int before = -1, after = -1;
    if (ioctl(receiver, FIONREAD, &before) < 0) die("before FIONREAD");
    errno = 0;
    ssize_t n;
    if (op == 0) n = read(receiver, buffer, request);
    else if (op == 1) n = recvfrom(receiver, buffer, request, 0, NULL, NULL);
    else {
        struct iovec iov[2] = { { buffer, valid }, { buffer + 4096, request - valid } };
        struct msghdr msg = { .msg_iov = iov, .msg_iovlen = op == 2 ? 1 : 2 };
        if (op == 2) iov[0].iov_len = request;
        n = recvmsg(receiver, &msg, 0);
    }
    int saved_errno = errno;
    if (ioctl(receiver, FIONREAD, &after) < 0) die("after FIONREAD");
    printf("%s: payload=%zu request=%zu valid=%zu ret=%zd errno=%d queued=%d->%d first=%c\n", name, payload, request, valid, n, saved_errno, before, after, buffer[0]);
    close(sender); close(receiver); close(listener); munmap(buffer, 8192);
}
int main(void)
{
