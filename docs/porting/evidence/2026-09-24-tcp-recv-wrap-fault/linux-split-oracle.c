...
static void run_case(const char *name, int op, size_t valid, size_t payload,
  ...
  if (sender < 0)
    die("sender socket");
  int nodelay = 1;
  if (setsockopt(sender, IPPROTO_TCP, TCP_NODELAY, &nodelay, sizeof(nodelay)) <
      0)
    die("TCP_NODELAY");
  ...
  size_t sent = 0;
  while (sent < payload) {
    size_t chunk = sent == 0 && payload > 3000 ? 3000 : payload - sent;
    ssize_t n = send(sender, payload_buffer + sent, chunk, 0);
    if (n <= 0)
      die("send");
    sent += (size_t)n;
    if (sent == 3000)
  ...
  for (int attempt = 0; attempt < 1000; attempt++) {
    if (ioctl(receiver, FIONREAD, &before) < 0)
      die("before FIONREAD");
    if (before == (int)payload)
      break;
    usleep(1000);
  }
  if (before != (int)payload)
    die("payload not queued");
  errno = 0;
  ssize_t n;
  if (op == 0)
