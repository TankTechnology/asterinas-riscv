...
- [x] Add a test backend that records read indices and immediately initializes each requested page. Its `read_page_async` body is:
...
- [x] Add `fault_around_reads_only_the_mapping_window`, following the adjacent `VmMapping::new`/`handle_page_fault` ktests. For each offset in `[0, 8, 120]`, create a fresh 128-page backend VMO, a read-only 16-page mapping, and a mapped VMO offset of `offset * PAGE_SIZE`. Use distinct 16-page-aligned virtual ranges in one active VmSpace to isolate cases without repeatedly activating address spaces. Invoke the real handler at mapping start. Assert:
...
- [x] Run the ktest before changing production code, retaining the failing log. From `kernel/` in the existing container:
...
- [x] Replace the faulty expression with:
...
- [x] Rerun the failing test, then all six mapping tests and the sequential-batching/concurrent-fault page-cache tests. OSDK selects exact function suffixes, not module substrings; zero-selection attempts are not counted. Check formatting and whitespace; commit the scoped fix after ordinary review (`830c91a68`).

## Task 2: Existing canary validation

- [x] Build an offline release candidate with the same features, preserving Image and bundle identity. Reuse existing Stage1, DTB, Debian and container.
- [x] Run QEMU Basic/Probe and the existing user-space clock probe on the frozen candidate.
- [x] Preflight RockOS SSH, unchanged default menu, unmounted Debian filesystem check, and exclusive serial ownership. Stage a separate candidate using `megrez_menu_board`, not by replacing an installed image.
- [x] Repeat Basic/Probe and full Desktop. Use the retained `desktop_observe.py` and `content_check.py` for the same window, page-JavaScript, DOM-handler and framebuffer criteria. Record cache policy and image hashes.
- [x] Request software shutdown once; observe firmware and recover RockOS. Record timeout failures honestly, even if later recovery succeeds. Do not issue a second forced reboot or reset.
- [x] Record before/after observations and remaining limitations in `docs/porting/evidence/2026-09-12-fault-window-and-rseq.md`, linked from the existing Firefox evidence document. Do not promote over the deployed networking kernel without source parity and the established promotion gates.
