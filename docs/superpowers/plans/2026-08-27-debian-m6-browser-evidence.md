...
**Goal:** Capture a foreground Baidu-hosted PNG in NetSurf and separately classify the packaged JavaScript engine while Asterinas runs the Debian M5 desktop in QEMU.
...
- [x] **Step 1: Write the failing profile and classifier tests**
...
- [x] **Step 3: Implement the minimal pure classifier contract**
...
- [x] **Step 4: Add `xdotool` to the immutable M5 profile and GREEN the tests**
...
- [x] **Step 1: Write failing fixture and session tests**

Require the entry fixture's static `ASTERINAS_JS_PENDING` title and inline
script-only navigation to a second local document with an
`ASTERINAS_JS_PASS` title and visible pass token. Require the M4 session to
pass `--enable_javascript=1` only when it accepts the trusted external URL file
and to preserve the quoted local-welcome fallback.

- [x] **Step 2: Verify RED and implement the fixture/session change**

Run the focused suite, confirm the missing fixture/options failures, make the
minimal changes, and rerun to GREEN.

- [x] **Step 3: Write failing executable-boundary evidence tests**

Use fake executable `xdotool`, `pgrep`, and a private proc-root. Test:

- one visible NetSurf window with the Baidu `result.png` title is activated;
...
- duplicate windows, oversized titles, command failures, and timeouts emit one
  line with the exact `DEBIAN_BROWSER_M6_FAIL reason=` prefix and a bounded
  lowercase reason token.

- [x] **Step 4: Verify RED and implement the bounded evidence script**
...
- [x] **Step 5: Install and order the M6 artifacts**

`configure_desktop_m5_network` installs the script as
`/usr/lib/asterinas/desktop-m6-browser-evidence`, the fixture under
the entry and pass fixtures under `/usr/share/asterinas`, and a root oneshot
service ordered after both
...
- [x] **Step 2: Verify RED and implement the thin subclass**
...
- [x] **Step 3: Add the explicit Make target and operator documentation**
...
- [x] **Step 4: Run focused and complete host verification**
...
- [x] **Step 1: Rebuild only the signed M5 rootfs**
...
- [x] **Step 2: Run one bounded M6 Asterinas QEMU gate**
...
- [x] **Step 3: Inspect the complete evidence set**

Require `passed:true`, monotonically ordered M5/M4/M6 markers, the resource-bound
Baidu PNG marker, one JavaScript status, matching final marker, exact slirp QEMU
argv, complete serial log, two 1280x1024 non-blank PPM files, and no surviving
QEMU process.

- [x] **Step 4: Run final verification, self-review once, commit, and push**
