...
- [x] Add a regression using a repository-style output path long enough to
  make the old monitor socket exceed the AF_UNIX limit.
- [x] Assert the session directory is private, under `/tmp`, and independent
  of the output path while boot/root hard links still reference the pinned
  output files.
- [x] Assert drain and launch failures remove the runtime directory.
- [x] Run the focused backend session tests and record RED.
- [x] Implement the minimal short-runtime-directory change.
- [x] Run focused GREEN plus Python/Ruff/diff checks.
- [x] Commit as `fix(riscv): bound Debian QEMU monitor paths`.
...
- [x] Add classifier RED cases for missing, duplicate, reordered, and
  failure-marked homepage/search evidence.
- [x] Add guest-script RED cases for exact homepage navigation, bounded title
  waits, frozen search interaction, and stable failures.
- [x] Preserve every M6 marker and add exact M7 homepage, search, and ready
  markers.
- [x] Add an M7 operation that waits for each capture point, captures two
  bounded PPMs, and publishes their metadata and bytes.
- [x] Add a focused Makefile unit target and run GREEN/static checks.
- [x] Commit as `test(riscv): define Debian Baidu page gate`.
...
- [x] Reuse the proven signed Debian/TUNA/cache workflow; do not change the
  mirror, package lock policy, or kernel.
- [x] Build one independent `desktop-m5-network` root image containing the M7
  evidence script.
- [x] Run the public rootfs `contract verify` command.
- [x] Record image, manifest, lock, InRelease, checksum, stage1, DTB, U-Boot,
  and kernel SHA-256 identities.

## Task 4: Run M6 regression and M7 real-page gate

- [x] Run M6 once at the documented long output path and require pass.
- [x] Run M7 once with the same generic-Sv39 SMP=4 QEMU contract.
...
- [x] Inspect the published homepage and failure screenshots visually; record
  what is visible without
  claiming unsupported modern-JavaScript behavior.
- [x] If M7 fails, classify it using the design failure boundaries and add only
  a focused reproducer before changing kernel code.
- [x] Clean the named container and the run-private QEMU directory used by
  this slice.
...
- [x] Document the M7 command, artifacts, success boundary, and known NetSurf
  limitations.
- [x] Mark the preserved M6 and actual-homepage checklist items accurately.
- [x] Run focused host tests, shell syntax checks, Python/Ruff checks, and
  `git diff --check` once.
- [x] Commit the documentation and report the exact QEMU evidence.
