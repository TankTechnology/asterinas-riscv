...
// Regression test for a stale timerfd readiness cache (PR #95).
FN_TEST(timerfd_read_clears_epoll_readiness)
{
	...
	TEST_SUCC(timerfd_settime(tfd, 0, &timer, NULL));

	TEST_RES(epoll_wait(epfd, &ev, 1, 1000),
		 _ret == 1 && ev.data.fd == tfd && (ev.events & EPOLLIN));
