...
cache_dir="$repo_dir/target/sched-info-model"
mkdir -p -- "$cache_dir"
...
run_case() {
    ...
    timeout --kill-after=5s 60s java -Xmx256m -XX:+UseSerialGC \
        ...
        -config "$model_dir/$config.cfg" "$model_dir/SchedInfo.tla" \
        > "$log" 2>&1 || status=$?
...
run_case ResetDuplicate 12 'Invariant LedgerMatches is violated.'
run_case CountBlocked 12 'Invariant LedgerMatches is violated.'
run_case Correct 0 'Model checking completed. No error has been found.'
printf 'PASS: two expected negative controls and the corrected scheduler ledger model.\n'
