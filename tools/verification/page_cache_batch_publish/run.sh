...
cache_dir="$repo_dir/target/page-cache-batch-publish-model"
mkdir -p -- "$cache_dir"
...
run_case() {
    ...
    timeout --kill-after=5s 60s java -Xmx256m -XX:+UseSerialGC \
        ...
        "$model_dir/PageCacheBatchPublish.tla" > "$log" 2>&1 || status=$?
...
run_case PublishBeforeLock 12 'Invariant NoUnsubmittedWaitCycle is violated.'
run_case LockBeforePublish 0 'Model checking completed. No error has been found.'
printf 'PASS: expected publish-before-lock counterexample and corrected finite model.\n'
