...
cache_dir="$repo_dir/target/riscv-cpu-local-migration-model"
mkdir -p -- "$cache_dir"
...
run_case() {
    ...
    timeout --kill-after=5s 60s java -Xmx256m -XX:+UseSerialGC \
        ...
        "$model_dir/RiscvCpuLocalMigration.tla" > "$log" 2>&1 || status=$?
...
printf 'PASS: two expected negative controls and the corrected finite model.\n'
