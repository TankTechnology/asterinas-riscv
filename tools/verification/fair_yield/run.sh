...
cache_dir="$repo_dir/target/fair-yield-model"
mkdir -p -- "$cache_dir"
...
run_case() {
    ...
    timeout --kill-after=5s 60s java -Xmx256m -XX:+UseSerialGC \
        ...
        -config "$model_dir/$config.cfg" "$model_dir/FairYield.tla" \
        > "$log" 2>&1 || status=$?
...
# Confirm regression sensitivity before accepting the corrected model.
run_case IgnoreYield 12 'Invariant ImmediateFairHandoff is violated.'
run_case ReenqueueBeforePick 12 'Invariant ImmediateFairHandoff is violated.'
run_case LoseOld 12 'Invariant NoLostRunnable is violated.'
run_case FairYield 0 'Model checking completed. No error has been found.'
printf 'PASS: three expected negative controls and the corrected finite model.\n'
