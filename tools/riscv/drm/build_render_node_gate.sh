...
usage() {
    cat <<'EOF'
Usage: build_render_node_gate.sh [OUTPUT]
       build_render_node_gate.sh --print-tools

Build the static RISC-V DRM render-node gate initramfs.
EOF
}
...
SOURCE="$SCRIPT_DIR/render_node_gate_init.c"
OUTPUT="${1:-$REPOSITORY_ROOT/target/drm-render-node/initramfs.cpio.gz}"
