...
usage() {
    cat <<'EOF'
Usage: build_gem_gate.sh [OUTPUT]
       build_gem_gate.sh --print-tools

Build the static RISC-V DRM GEM gate initramfs.
EOF
}
...
SOURCE="$SCRIPT_DIR/gem_gate_init.c"
OUTPUT="${1:-$REPOSITORY_ROOT/target/drm-gem/initramfs.cpio.gz}"
