...
usage() {
    cat <<'EOF'
Usage: build_virgl_param_gate.sh [OUTPUT]
       build_virgl_param_gate.sh --print-tools

Build the static RISC-V DRM virgl-parameter gate initramfs.
EOF
}
...
SOURCE="$SCRIPT_DIR/virgl_param_gate_init.c"
OUTPUT="${1:-$REPOSITORY_ROOT/target/drm-virgl/initramfs.cpio.gz}"
