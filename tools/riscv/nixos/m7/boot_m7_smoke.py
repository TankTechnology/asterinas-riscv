...
"""Boot the M7 SCM_RIGHTS + SO_PEERCRED repro initramfs on Asterinas RISC-V.

Same QEMU virt / U-Boot booti handoff as boot_m4_smoke.py. The static /init
runs three checks (see scm_repro.c):
  1. socketpair(AF_UNIX) -> __M7_SOCKETPAIR_OK__
  2. SO_PEERCRED      -> __M7_PEERCRED_OK__
  3. sendmsg(SCM_RIGHTS) fd pass + recvmsg read-back -> __M7_SCM_RIGHTS_OK__
"""
...
INIT_MARKER = b">>> M7 init: SCM_RIGHTS + SO_PEERCRED repro <<<"
DONE_MARKER = b">>> M7 repro done <<<"

CHECKS = [
    ("socketpair AF_UNIX", b"__M7_SOCKETPAIR_OK__"),
    ("SO_PEERCRED", b"__M7_PEERCRED_OK__"),
    ("SCM_RIGHTS fd pass", b"__M7_SCM_RIGHTS_OK__"),
]
...
def main() -> int:
    ...
    parser.add_argument("--serial-log", type=Path, default=Path("/tmp/asterinas-m7-serial.log"))
