...
"""Boot the FOUNDATION-M4 smoke test on Asterinas RISC-V and report pass/fail.

Mirrors ``boot_ltp_gate.py``. The initramfs ``/init`` runs each M3 test and
prints per-test ``[FM4]`` lines plus ``__FM4_<NAME>_{OK,FAIL}__`` markers, and
finishes with ``__FM4_DONE__`` followed by ``__FM4_PASS__`` / ``__FM4_FAIL__``.
"""
...
DONE_MARKER = b"__FM4_DONE__"
PASS_MARKER = b"__FM4_PASS__"
FAIL_MARKER = b"__FM4_FAIL__"
...
def main() -> int:
    ...
    parser.add_argument("--serial-log", type=Path, default=Path("/tmp/asterinas-fm4-serial.log"))
    ...
    try:
        ...
        print("[boot] waiting for FM4 completion", flush=True)
        try:
            ...
            print("[fm4] FAIL: no completion marker (hang/crash)", flush=True)
    ...
    print("\n=== FOUNDATION-M4 result ===", flush=True)
    for line in text.splitlines():
        if line.startswith("[FM4]") or line.startswith("__FM4") or line.startswith(">>>"):
            print(line, flush=True)

    print(f"\n=== FOUNDATION-M4: {'PASS' if passed else 'FAIL'} (smp={args.smp}) ===", flush=True)
