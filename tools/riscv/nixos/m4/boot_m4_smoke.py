...
"""Boot the M4 clone(CLONE_SETTLS)+SIGSEGV repro initramfs on Asterinas RISC-V.

Same QEMU virt / U-Boot booti handoff as boot_m2_smoke.py. The static /init
runs two checks (see tls_repro.c):
  1. raw clone(CLONE_THREAD|CLONE_SETTLS); child reads tp and writes tp[1]
     (musl's DTV slot). Pass = __M4_TLS_OK__, fail = __M4_TLS_FAIL__.
  2. fork(); child dereferences unmapped 0x8; parent expects SIGSEGV death.
     Pass = __M4_SEGV_OK__, fail = __M4_SEGV_FAIL__.
"""
...
def main() -> int:
    ...
    parser.add_argument("--serial-log", type=Path, default=Path("/tmp/asterinas-m4-serial.log"))
    ...
    try:
        ...
        try:
            ...
            print("[smoke] did not reach __M4_DONE__ (crash/hang)", flush=True)
