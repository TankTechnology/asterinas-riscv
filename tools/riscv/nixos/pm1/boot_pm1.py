...
"""Boot the POLISH-M1 smoke test on Asterinas RISC-V and report pass/fail.

Mirrors ``boot_ltp_gate.py``. The initramfs ``/init`` runs each M3 test and
prints per-test ``[PM1]`` lines plus ``__PM1_<NAME>_{OK,FAIL}__`` markers, and
finishes with ``__PM1_DONE__`` followed by ``__PM1_PASS__`` / ``__PM1_FAIL__``.
"""
...
DONE_MARKER = b"__PM1_DONE__"
PASS_MARKER = b"__PM1_PASS__"
FAIL_MARKER = b"__PM1_FAIL__"
...
def main() -> int:
    ...
    parser.add_argument("--serial-log", type=Path, default=Path("/tmp/asterinas-pm1-serial.log"))
    ...
    try:
        ...
        print("[boot] waiting for PM1 completion", flush=True)
        try:
            ...
            print("[pm1] FAIL: no completion marker (hang/crash)", flush=True)
    ...
    print("\n=== POLISH-M1 result ===", flush=True)
    for line in text.splitlines():
        if line.startswith("[PM1]") or line.startswith("__PM1") or line.startswith(">>>"):
            print(line, flush=True)

    print(f"\n=== POLISH-M1: {'PASS' if passed else 'FAIL'} (smp={args.smp}) ===", flush=True)
