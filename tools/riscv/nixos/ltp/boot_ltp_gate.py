...
"""Boot the LTP syscall gate on Asterinas RISC-V and report pass/fail.

Reuses the repository's U-Boot ``booti`` handoff (see the other
``tools/riscv/nixos/*/boot_*_smoke.py`` drivers). The initramfs ``/init`` execs
``/ltp_runner``, which streams per-test ``[VERDICT]`` lines and finishes with a
``__LTP_GATE_DONE__`` line followed by ``__LTP_GATE_PASS__`` or
``__LTP_GATE_FAIL__``. This driver boots to those markers, dumps the summary,
...
DONE_MARKER = b"__LTP_GATE_DONE__"
PASS_MARKER = b"__LTP_GATE_PASS__"
...
def main() -> int:
    ...
    try:
        ...
        print("[boot] waiting for LTP gate completion", flush=True)
        try:
            boot.read_until(DONE_MARKER, args.command_timeout)
            # The runner emits the summary + a PASS/FAIL marker right after DONE.
            tail = boot.read_until_any([PASS_MARKER, FAIL_MARKER], 120)
        except TimeoutError:
            tail = bytes(boot.transcript[-4000:])
            print("[gate] FAIL: no completion marker (hang/crash)", flush=True)
    ...
    # Print the per-test verdict lines and the summary.
    print("\n=== LTP gate result ===", flush=True)
    for line in text.splitlines():
        if line.startswith("[") or line.startswith("__LTP_GATE") or "summary" in line:
            print(line, flush=True)

    print(f"\n=== LTP gate: {'PASS' if passed else 'FAIL'} (smp={args.smp}) ===", flush=True)
