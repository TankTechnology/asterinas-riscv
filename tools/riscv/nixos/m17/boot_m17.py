#!/usr/bin/env python3
"""Boot the DRM kernel with the M17 initramfs (atomictest as /init) and
verify the atomic-modesetting ioctl surface.

Evidence protocol over the serial console:
  M17_PASS <check...> / M17_FAIL <check...> / M17_ALL_PASS / M17_FAILED

Artifacts (boot.ext4, initramfs, dtb, u-boot) live in target/drm-m17/,
built by tools/riscv/nixos/m17/build_m17.sh.
"""
import os
import pty
import select
...
WORKDIR = Path(__file__).resolve().parents[4] / "target" / "drm-m17"
...
def run():
    ...
    print("[boot] Waiting for kernel (M17_ALL_PASS / M17_FAILED)...")
    ...
    while time.time() < deadline:
        ...
        if r:
            ...
            sys.stdout.buffer.write(data)
            sys.stdout.buffer.flush()
            if b"M17_ALL_PASS" in output or b"M17_FAILED" in output:
                print("[boot] M17 verdict received")
                break
...
def main():
    print("[boot] Launching QEMU (DRM kernel, M17 initramfs)...")
    output = run()
    text = output.decode("utf-8", errors="replace")
    print("\n=== M17 Atomic Modesetting Results ===")
    passes = failures = 0
    for line in text.split("\n"):
        if "M17_PASS" in line or "M17_FAIL" in line:
            # Strip kernel log interleaving: keep the M17_* payload.
            idx = line.find("M17_")
            print(f"  {line[idx:]}")
            passes += line.count("M17_PASS")
            failures += line.count("M17_FAIL")
    print(f"\nSummary: PASS={passes} FAIL={failures}")
    if b"M17_ALL_PASS" in output and failures == 0:
        print("M17_ATOMIC_PASS: all atomic modesetting checks passed")
        return 0
    print("M17_ATOMIC_FAIL")
    return 1
