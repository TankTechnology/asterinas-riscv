...
# Assemble the DRM-M17 atomic-modesetting test initramfs and pack a fresh
# boot disk. The in-guest init is `atomictest` (statically linked, raw
# ioctls) which prints M17_* evidence lines on the serial console.
#
# Usage:
#     bash build_m17.sh
...
BUILD_ROOT="${REPO_ROOT}/target/nixos/drm-m17"
ROOTFS="${BUILD_ROOT}/rootfs"

DISK_DIR="${REPO_ROOT}/target/drm-m17"
...
"${CC}" -O2 -static -no-pie -fno-stack-protector \
    -o "${ROOTFS}/init" "${SRC_DIR}/atomictest.c"
