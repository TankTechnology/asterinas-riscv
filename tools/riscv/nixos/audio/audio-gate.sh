...
# One-command AUDIO-M1 gate: build the sine-wave initramfs, re-pack the U-Boot
# boot disk with the current kernel + initramfs + DTB, then boot it in QEMU
# with a virtio-sound device and verify the host WAV backend received the PCM.
...
BOOT_DRIVER="${AUDIO_DIR}/boot_audio.py"
BUILD_SCRIPT="${AUDIO_DIR}/build_audio.sh"

KERNEL_IMAGE="${ASTERINAS_RISCV_BOOTI:-${REPO_ROOT}/target/osdk/aster-kernel-osdk-bin.Image}"
INITRAMFS="${REPO_ROOT}/target/nixos/audio/audio-initramfs.cpio.gz"
BOOT_DISK="${REPO_ROOT}/target/qemu-uboot/current/boot.ext4"
DTB="${REPO_ROOT}/target/qemu-uboot/current/qemu-virt.dtb"
SERIAL_LOG="${REPO_ROOT}/target/nixos/audio/audio-serial.log"
WAV_OUT="${REPO_ROOT}/target/nixos/audio/audio-out.wav"
