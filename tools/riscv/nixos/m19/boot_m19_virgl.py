#!/usr/bin/env python3
"""Boot DRM kernel (M19) with virtio-gpu-gl + host virglrenderer and run the
in-guest EGL/GLES2 verification client (Alpine rootfs initramfs).

Evidence protocol over the serial console:
  M19_GBM_BACKEND / M19_EGL_DISPLAY_OK / M19_EGL_CTX_OK
  M19_GL_VENDOR / M19_GL_RENDERER / M19_GL_VERSION
  M19_FRAME <n> csum=<hex> distinct_ge=<n>
  M19_FRAME_SAVED, M19_PPM_BASE64_BEGIN..END (decoded to evidence/),
  M19_EGL_DONE / M19_EGL_FAIL <stage>, M19_VERIFY_DONE

Artifacts (boot.ext4, initramfs, dtb, u-boot) live in target/drm-m19/.
Evidence (serial log + decoded PPM) lands in target/drm-m19/evidence/.
"""
import base64
import os
import pty
...
def run() -> bytes:
    ...
    while time.time() < deadline:
        ...
        if r:
            ...
            sys.stdout.buffer.write(data)
            sys.stdout.buffer.flush()
            if b"M19_VERIFY_DONE" in output or b"M19_EGL_FAIL" in output:
                break
...
def main() -> int:
    ...
    print("[boot] Launching QEMU (M19 virgl end-to-end)...")
    output = run()
    text = output.decode("utf-8", errors="replace")

    (EVIDENCE / "m19-virgl-serial.log").write_text(text)

    print("\n=== M19 virgl EGL Results ===")
    markers = {}
    frames = []
    for line in text.splitlines():
        line = line.strip()
        if "M19_GBM_BACKEND" in line:
            markers["gbm"] = line
        elif "M19_EGL_DISPLAY_OK" in line:
            markers["display"] = line
        elif "M19_EGL_CTX_OK" in line:
            markers["ctx"] = line
        elif "M19_GL_" in line:
            key = line.split()[0]
            markers[key] = line
        elif "M19_FRAME " in line:
            frames.append(line)
        elif "M19_EGL_DONE" in line:
            markers["done"] = line
        elif "M19_EGL_FAIL" in line:
            markers["fail"] = line
        elif "M19_VERIFY_DONE" in line:
            markers["verify"] = line
    ...
    m = re.search(
        r"M19_PPM_BASE64_BEGIN\s*\r?\n(.*?)M19_PPM_BASE64_END", text, re.S)
    ppm_ok = False
    if m:
        ...
        try:
            ...
            if ppm.startswith(b"P6"):
                (EVIDENCE / "m19_frame.ppm").write_bytes(ppm)
                ppm_ok = True
                print(f"  PPM decoded: {len(ppm)} bytes -> {EVIDENCE}/m19_frame.ppm")
        except Exception as e:
            print(f"  PPM decode error: {e}")

    kmscube_frames = len(re.findall(r"Rendered \d+ frames", text))
    print(f"  kmscube rendered reports: {kmscube_frames}")
    # The PPM dump is best-effort (kernel log lines can interleave with the
    # base64 stream on the serial console); it is not part of the verdict.
    passed = (
        "done" in markers
        and "fail" not in markers
        and len(frames) >= 2
        and "M19_GL_RENDERER" in markers
        and kmscube_frames >= 2
    )
    ...
    if passed:
        ...
        if len(csums) < 2:
            ...
            passed = False
    print("M19_VIRGL_PASS" if passed else "M19_VIRGL_FAIL")
