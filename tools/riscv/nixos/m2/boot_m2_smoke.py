...
"""Boot the M2 dynamically linked musl initramfs on Asterinas RISC-V.

Same QEMU virt / U-Boot booti handoff as boot_busybox_smoke.py, but the
initramfs /init runs /bin/hello_dyn (dynamic musl binary with a DT_NEEDED
shared library), then busybox sh prints the final marker. Verifies:
  1. execve of an ET_DYN binary works,
  2. the musl loader (ld-musl-riscv64.so.1) runs,
  3. DT_NEEDED resolution loads libgreet.so,
  4. the M1 static busybox world still coexists in the same initramfs.
"""
...
INIT_MARKER = b">>> M2 init: dynamic musl smoke <<<"
FINAL_MARKER = b"__M2_SHELL_OK__"

# Each entry: (name, needle that proves the step produced correct output).
CHECKS = [
    ("hello_dyn (exec ET_DYN)", b"__M2_HELLO_DYN__ hello from libgreet to riscv64"),
    ("libgreet.so (DT_NEEDED)", b"libgreet"),
    ("busybox sh (static world)", b"__M2_SHELL_OK__"),
]
...
def main() -> int:
    ...
    parser.add_argument("--serial-log", type=Path, default=Path("/tmp/asterinas-m2-serial.log"))
    ...
    try:
        ...
        try:
            ...
            print("[smoke] did not reach final marker (crash/hang)", flush=True)
    ...
    print("\n=== M2 smoke results ===", flush=True)
    for r in results:
        print(f"  {r['command']}: {r['status']}", flush=True)
    if final_tail:
        print("\n--- serial tail ---", flush=True)
        print(final_tail.decode(errors="replace")[-2000:], flush=True)
