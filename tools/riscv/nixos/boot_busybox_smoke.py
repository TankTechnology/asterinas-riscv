...
"""Boot the M1 BusyBox initramfs on Asterinas RISC-V and smoke-test commands.

Drives the same QEMU ``-machine virt`` / U-Boot ``booti`` handoff as the
repository's other RISC-V runners, but headless. The initramfs ``/init`` runs a
fixed non-interactive smoke script (``busybox sh -c ...``) that exercises sh,
ls, cat, mount and ps; this driver boots to that script, collects the serial
transcript, and reports each command pass/fail so a kernel panic can be
attributed to the exact command/syscall that triggered it.

Single-threaded serial I/O (``selectors``), matching qemu_uboot_session.py.

Usage:
    python3 tools/riscv/nixos/boot_busybox_smoke.py \
        [--serial-log /tmp/m1-serial.log] [--command-timeout 60]
"""
...
INIT_MARKER = b">>> M1 busybox init: running smoke script <<<"
FINAL_MARKER = b"__M1_DD_DONE__"

# Each entry: (name, needle that proves the command produced correct output).
CHECKS = [
    ("sh",    b"__M1_SH_OK__"),
    ("ls",    b"__M1_LS_DONE__"),
    ("cat",   b"hello-m1"),
    ("mount", b"__M1_MOUNT_DONE__"),
    ("ps",    b"__M1_PS_DONE__"),
    ("uname", b"__M1_UNAME_DONE__"),
    ("stat",  b"__M1_STAT_DONE__"),
    ("df",    b"__M1_DF_DONE__"),
    ("free",  b"__M1_FREE_DONE__"),
    ("cpuinfo", b"__M1_CPUINFO_DONE__"),
    ("proc-self", b"__M1_SELF_DONE__"),
    ("dd",    b"__M1_DD_DONE__"),
]

# U-Boot command sequence (headless generic-sv39 virt boot).
UBOOT_COMMANDS = [
...
def main() -> int:
    ...
    parser.add_argument("--serial-log", type=Path, default=Path("/tmp/asterinas-m1-serial.log"))
    ...
    try:
        ...
        print("[boot] waiting for init smoke script", flush=True)
        boot.read_until(INIT_MARKER, 120)
        print("[ok] init reached smoke script", flush=True)

        try:
            boot.read_until(FINAL_MARKER, args.command_timeout)
        except TimeoutError:
            final_tail = bytes(boot.transcript[-3000:])
            print("[smoke] smoke script did not reach final marker (crash/hang)", flush=True)
            results.append({"command": "smoke", "status": "CRASH/HANG", "tail": final_tail})
            raise

        # All commands ran; verify each produced its expected output.
