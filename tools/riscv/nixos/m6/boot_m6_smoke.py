...
"""Boot the M6 Nix-build initramfs on Asterinas RISC-V.

Same QEMU virt / U-Boot booti handoff as boot_m3_smoke.py, but /init runs a
`nix build` smoke script instead of `nix eval`:
  1. trivial derivation — `nix build` of a builtins.derivation whose builder is
     /bin/sh writing a fixed string to $out, asserting the output lands in
     /nix/store and reads back.
  2. (step 2) hello — `nix build` compiling hello.c in the guest with the
