...
"""NIXOS-N1 guest gate: boot the netlink-probe initramfs on Asterinas RISC-V.
...
Usage:
    python3 tools/riscv/nixos/n1/boot_n1_smoke.py \
        [--serial-log /tmp/n1-netlink/serial.log] [--command-timeout 120]
"""
...
BOOT_DISK = Path(os.environ.get("N1_DISK_DIR", "/tmp/n1-netlink")) / "boot.ext4"

KERNEL_LOAD = 0x8020_0000
INITRD_LOAD = 0x8300_0000
DTB_LOAD = 0x8800_0000

INIT_MARKER = b">>> N1 init: running netlink script <<<"
FINAL_MARKER = b"__N1_DONE__"

# Each entry: (name, needle that proves the step produced correct output).
CHECKS = [
    ("uevent-socket", b"__NL_UEVENT_OK__"),
    ("rtm-getlink", b"__NL_GETLINK_OK__"),
    ("rtm-getaddr", b"__NL_GETADDR_OK__"),
    ("rtm-newaddr", b"__NL_NEWADDR_OK__"),
    ("probe-all-ok", b">>> N1 probe: ALL OK <<<"),
    ("ip-link", b"__N1_IP_LINK__"),
    ("ip-link-eth0", b"eth0"),
    ("ip-addr", b"__N1_IP_ADDR__"),
    ("ip-addr-lo", b"127.0.0.1"),
]
...
def main() -> int:
    ...
    parser.add_argument("--serial-log", type=Path, default=Path("/tmp/n1-netlink/serial.log"))
    ...
    try:
        ...
        print("[boot] waiting for N1 init script", flush=True)
