...
Usage:
    python3 tools/riscv/nixos/n1/boot_n1_smoke.py \
        [--serial-log /tmp/n3-nix/serial.log] [--command-timeout 600]
"""
...
BOOT_DISK = Path(os.environ.get("N3_DISK_DIR", "/tmp/n3-nix")) / "boot.ext4"

KERNEL_LOAD = 0x8020_0000
INITRD_LOAD = 0x8300_0000
DTB_LOAD = 0x8800_0000

INIT_MARKER = b">>> N3 init: running nix-daemon script <<<"
FINAL_MARKER = b"__N3_DONE__"

# Each entry: (name, needle that proves the step produced correct output).
CHECKS = [
    ("nix-version", b"nix (Nix) 2.30.2"),
    ("nix-version-exit", b"__N3_VERSION_EXIT__=0"),
    ("daemon-ping", b"__N3_PING_EXIT__=0"),
    ("profile-install", b"__N3_INSTALL_EXIT__=0"),
...
def main() -> int:
    ...
    parser.add_argument("--serial-log", type=Path, default=Path("/tmp/n3-nix/serial.log"))
