...
"""Boot the System V shared-memory smoke test on Asterinas RISC-V.

Drives the same QEMU ``-machine virt`` / U-Boot ``booti`` handoff as the
repository's other RISC-V runners. The initramfs ``/init`` execs the static
``/shm_smoke`` test; this driver boots to that test, collects the serial
transcript, and reports pass/fail on the ``__SYSV_SHM_OK__`` marker.
"""
...
OK_MARKER = b"__SYSV_SHM_OK__"
FAIL_MARKER = b"___SYSV_SHM_FAIL__"
...
def main() -> int:
    ...
    parser.add_argument("--serial-log", type=Path, default=Path("/tmp/asterinas-shm-serial.log"))
    ...
    try:
        ...
        print("[boot] waiting for shm smoke test result", flush=True)
        try:
            boot.read_until(OK_MARKER, args.command_timeout)
            print("[smoke] PASS: __SYSV_SHM_OK__", flush=True)
        except TimeoutError:
            tail = bytes(boot.transcript[-3000:])
            print("[smoke] FAIL: no OK marker (crash/hang)", flush=True)
            print(tail.decode("utf-8", "replace")[-2500:], flush=True)
            return 1
    finally:
        boot.close()

    print("\n=== sysv shm smoke result ===", flush=True)
    print("  sysv_shm: PASS", flush=True)
    return 0
