...
# M8 capability repro: assemble the pivot_root / mount-ns / cgroup2 / devtmpfs
# probe as a static /init, pack a tiny initramfs, and repack the QEMU boot disk
# with the current kernel Image + this initramfs. Mirrors the M7 minimal-repro
# flow.
...
BUILD_ROOT="${NIXOS_ROOT}/m8"
ROOTFS="${BUILD_ROOT}/cap-rootfs"
OUTPUT="${1:-${BUILD_ROOT}/m8-cap-initramfs.cpio}"
...
# 1. Static /init (glibc static, same pattern as M1-M7).
"${CC_STATIC}" -O2 -static -no-pie -fno-stack-protector \
    -o "${BUILD_ROOT}/init" "${SRC_DIR}/cap_repro.c"
...
STAGE="${REPO_ROOT}/target/qemu-uboot/current/.m8-cap-stage"
