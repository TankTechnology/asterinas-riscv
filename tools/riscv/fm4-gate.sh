...
FM4_DIR="${REPO_ROOT}/tools/riscv/nixos/fm4"
BOOT_DRIVER="${FM4_DIR}/boot_fm4.py"
BUILD_SCRIPT="${FM4_DIR}/build_fm4.sh"

KERNEL_IMAGE="${ASTERINAS_RISCV_BOOTI:-${REPO_ROOT}/target/osdk/aster-kernel-osdk-bin.Image}"
INITRAMFS="${REPO_ROOT}/target/nixos/fm4/fm4-initramfs.cpio.gz"
BOOT_DISK="${REPO_ROOT}/target/qemu-uboot/current/boot.ext4"
DTB="${REPO_ROOT}/target/qemu-uboot/current/qemu-virt.dtb"
SERIAL_LOG="${REPO_ROOT}/target/nixos/fm4/fm4-serial.log"
...
echo ""
echo "===== FOUNDATION-M4: SMP=${SMP} ====="
if python3 "${BOOT_DRIVER}" --smp "${SMP}" --serial-log "${SERIAL_LOG}.smp${SMP}"; then
    echo "===== FOUNDATION-M4: PASS ====="
else
    echo "===== FOUNDATION-M4: FAIL ====="
    exit 1
fi
