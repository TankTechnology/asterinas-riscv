...
def _require_success(connection: socket.socket) -> None:
...
def _send_command(connection: socket.socket, command: dict[str, object]) -> None:
    connection.sendall(json.dumps(command, separators=(",", ":")).encode("utf-8") + b"\n")


def _read_output(output_path: Path) -> bytes:
...
def capture_screendump(
    ...
    *,
    ...
    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as connection:
        ...
        greeting = _read_message(connection)
        if "QMP" not in greeting:
            raise ValueError("QMP greeting is missing QMP")
        _send_command(connection, {"execute": "qmp_capabilities"})
