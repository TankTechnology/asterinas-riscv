...
# One-command SYSTEMD-DESKTOP-M1 gate: assemble the systemd+desktop initramfs,
# re-pack the U-Boot boot disk with the current kernel + initramfs + DTB, then
# boot it in QEMU (bochs framebuffer chain) and report how far systemd got.
#
# Usage:
#   tools/riscv/systemd/gate_desktop.sh                  # build initramfs + boot
#   tools/riscv/systemd/gate_desktop.sh --collect-timeout 300
...
SERIAL_LOG="${REPO_ROOT}/target/systemd-desktop/serial.log"
