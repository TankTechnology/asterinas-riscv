...
# One-command NIXOS-STAGE2-M1 gate: assemble the systemd + desktop + nix-profile
# initramfs, re-pack the U-Boot boot disk with the current kernel + initramfs +
# DTB, then boot it in QEMU (bochs framebuffer chain) and report systemd +
# desktop + nix smoke results.
#
# Usage:
#   tools/riscv/systemd/gate_nixos.sh                  # build initramfs + boot
#   tools/riscv/systemd/gate_nixos.sh --collect-timeout 300
...
SERIAL_LOG="${REPO_ROOT}/target/systemd-desktop/serial-nixos.log"
