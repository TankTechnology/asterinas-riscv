...
ready() {
    ...
    pgrep -u "$USER_ID" -f '(^|/)matchbox-window-manager([[:space:]]|$)' \
        >/dev/null || return 1
    pgrep -u "$USER_ID" -x pcmanfm >/dev/null || return 1
    ...
    [[ "$window_tree_lower" == *"asterinas files"* ]] || return 1
...
emit "DEBIAN_DESKTOP_M4_CLIENTS window-manager=matchbox file-manager=pcmanfm browser=netsurf terminal=xterm"
