...
readonly SESSION_LOG="$HOME/desktop-m3-session.log"
...
{
    printf 'DESKTOP_M3_DEVICE identity='
    /usr/bin/id
    shopt -s nullglob
    readonly input_devices=(/dev/input/event*)
    /usr/bin/stat -c 'DESKTOP_M3_DEVICE path=%n uid=%u gid=%g mode=%a type=%F' \
        /dev/fb0 "${input_devices[@]}"
    if exec 9<>/dev/fb0; then
        printf '%s\n' 'DESKTOP_M3_DEVICE framebuffer-open-rw=ok'
        exec 9>&-
    else
        printf '%s\n' 'DESKTOP_M3_DEVICE framebuffer-open-rw=failed'
    fi
} >>"$SESSION_LOG" 2>&1
