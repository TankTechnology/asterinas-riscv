...
fn do_change_type(target_path: Path, flags: MountFlags, ctx: &Context) -> Result<()> {
    ...
    // Record the requested propagation type on the mount (and, if `MS_REC`, on
    // its children). Event propagation between peer groups is not yet
    // implemented, so this is the type bookkeeping without the cross-mount
    // mirroring that Linux performs.
    let prop_type = if flags.contains(MountFlags::MS_PRIVATE) {
